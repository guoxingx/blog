...
// GetURLLastPart returns the last part of url separated by /
func GetURLLastPart(req *http.Request) string {
	res := SeparateURL(req)
	return res[len(res)-1]
}

// SeparateURL returns an slice of separated url with /
func SeparateURL(req *http.Request) []string {
	return strings.Split(req.URL.EscapedPath(), "/")
}
